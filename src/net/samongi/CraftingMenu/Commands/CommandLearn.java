...
public class CommandLearn extends BaseCommand
{
  ...
  public boolean run(CommandSender sender, String[] args)
  {
    if(args.length == 1 && sender instanceof Player)
    {
      ...
      recipe.learnRecipe((Player) sender);
      sender.sendMessage(ChatColor.YELLOW + "You have learned the recipe '" + ChatColor.AQUA + recipe.getName() + ChatColor.YELLOW + "'");
    }
    else if(args.length == 2)
    {
      ...
      sender.sendMessage(ChatColor.YELLOW + "'" + ChatColor.GREEN + player.getName() + ChatColor.YELLOW + " has learned the recipe '" + ChatColor.AQUA + recipe.getName() + ChatColor.YELLOW + "'");
      if(sender != player) player.sendMessage(ChatColor.YELLOW + "You have learned the recipe '" + ChatColor.AQUA + recipe.getName() + ChatColor.YELLOW + "'");
      return true;
    }
    return true;
  }
